"""化验数据接口：维护化验结果，覆盖录入结果、审核通过、退回修改等动作。"""
...
def list_entries(
    ...
    status: str | None = Query(default=None, description="待录入、已录入、已审核、已退回"),
    page: int = 1,
    size: int = 20,
) -> PageResult[dict]:
    """按化验编号与状态过滤化验数据列表；没有数据时返回空页，不报错。"""
    if size > 200:
